...
[`@argo/tool-executor/tool-result`](../../packages/tool-executor/README.md). The
...
not a bare `Error`. `ToolError` (`@argo/tool-executor/tool-result`) and the
loop's wider `LoopToolError` (`@argo/agent-loop/loop-tool-result`) are the
