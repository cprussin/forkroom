...
- **All packages building UI MUST use [Panda CSS](https://panda-css.com).**
  ...
  them.
- **All packages MUST extend the `@forkroom/component-library` Panda preset.**
...
- **All packages MUST use components from `@forkroom/component-library` where
...
1. Add the catalog dependencies to the package's `devDependencies`
   ...
   (`@forkroom/component-library` as `workspace:*`).
...
`marginInlineEnd: -2`. The argo preset extends the default Panda scale
...
Stick to the **semantic** color tokens defined by the argo preset:
...
`@forkroom/component-library` preset (`pandacss-preset.ts`) when it is
...
Every primitive semantic color in the argo preset has both dark (`base`)
...
The argo preset also defines `_activeEnabled` and `_hoverEnabled`
